...
import (
	...
	"github.com/NodePrime/ericsson-hds-agent/agent/log"
)
...
func dryRun(context *Agent, initialFreq int) {
	...
	log.Consolef("running in 'dry run' mode, it will take %d seconds...", dryRunSeconds)
...
func dryRunResults(stdout, stderr string, frequency int) (err error) {
	...
	fmt.Printf("%d items (%d bytes) are going to be sent during one iteration\n",
		itemsSum, bytesSum)

	estimatedBytes, estimatedItems := projectDataSize(bytesSum, itemsSum, frequency)
	fmt.Printf("%d items (%d bytes) are going to be sent over one hour since we have -frequency %d seconds\n",
